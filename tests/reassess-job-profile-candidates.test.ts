...
import { InMemoryAIAssessmentAuditSink, InMemorySearchRunRepository } from "../src/infrastructure/memory/in-memory-repositories.js";
...
test("批量重新评估覆盖历史候选人的旧业务评估结果", async () => {
  const jobProfile = {
    ...createConfirmedJobProfile(),
    currentVersionId: "job-1-v2",
  };
  ...
  assert.equal(savedRun?.candidates.every((candidate) => candidate.matchAssessment?.jobProfileVersionId === "job-1-v2"), true);
  assert.equal(savedRun?.candidates.some((candidate) => candidate.matchAssessment?.score === 40), false);
