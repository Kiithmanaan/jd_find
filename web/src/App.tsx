...
export function App(): React.JSX.Element {
  ...
  const createRun = useMutation({ mutationFn: async () => {
    const version = versions.data?.versions.find((item) => item.id === versions.data.currentVersionId && item.status === "Confirmed");
    if (!version) throw new Error("未找到当前已确认画像版本。");
    return realApi.createRun(version, target);
  ...
  return <main className="mx-auto max-w-6xl p-6"><header className="mb-6 flex items-center justify-between"><h1 className="text-2xl font-semibold">JD Search 工作台</h1><button onClick={() => { realApi.logout(); setAuthenticated(false); }}>退出</button></header>
    <section className="mb-5 grid gap-3 rounded border p-4 md:grid-cols-4"><input className="rounded border p-2 md:col-span-2" placeholder="JobProfile ID" value={profileId} onChange={(e) => setProfileId(e.target.value)} /><input className="rounded border p-2" type="number" min={10} max={500} value={target} onChange={(e) => setTarget(Number(e.target.value))} /><button className="rounded bg-black px-3 py-2 text-white" disabled={!versions.data || createRun.isPending} onClick={() => createRun.mutate()}>启动插件寻访</button><input className="rounded border p-2 md:col-span-2" placeholder="SearchRun ID" value={runId} onChange={(e) => setRunId(e.target.value)} /><button className="rounded border px-3 py-2" disabled={!runId} onClick={() => cancel.mutate()}>取消任务</button><button className="rounded border px-3 py-2" disabled={!profileId} onClick={() => reassess.mutate()}>按当前版本重评估</button></section>
