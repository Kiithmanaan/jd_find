...
| 生产错误日志 | 自动化测试（`tests/api.test.ts`） | API 未捕获异常时 `setErrorHandler` 至少输出一行结构化 stderr 日志（含请求方法、路径、SearchRun id、错误名称与消息） | 开发中 |
...
| 插件候选人提交限流 | 自动化测试（`tests/api.test.ts`、`tests/rate-limiter.test.ts`） | 同一 Plugin Token + SearchRun 超过阈值时返回 `429 RateLimited` 并带 `retryAfterSeconds` | 开发中 |
| 插件附件上传限流 | 自动化测试（`tests/api.test.ts`、`tests/rate-limiter.test.ts`） | 同一 Plugin Token + SearchRun 超过阈值时返回 `429 RateLimited` 并带 `retryAfterSeconds` | 开发中 |
