# 第一阶段验收清单

## 1. 基础工程

| 验收项 | 验证方式 | 通过标准 | 当前状态 |
|---|---|---|---|
| 后端类型检查 | `npm run typecheck` | 命令通过 | 已完成 |
| 前端类型检查 | `npm run web:typecheck` | 命令通过 | 已完成 |
| Prisma schema 校验 | `DATABASE_URL=... npm run prisma:validate` | 命令通过 | 已完成 |
| 自动化测试 | `npm test` | 全部测试通过 | 已完成 |
| 前端构建 | `npm run web:build` | 构建成功 | 已完成 |
| CI | GitHub Actions | push 后执行全量检查 | 已完成 |

## 2. 认证与账号

| 验收项 | 验证方式 | 通过标准 | 当前状态 |
|---|---|---|---|
| 创建用户 | `npm run user:create` | 用户写入数据库 | 已完成 |
| Web 登录 | `POST /api/auth/login` | 返回 Web JWT | 已完成 |
| 插件登录 | `POST /api/plugin/auth/login` | 返回 Plugin Token | 已完成 |
| Token 过期提示 | 前端交互验收 | 过期后提示重新登录 | 待实现 |

## 3. SearchRun 与插件提交

| 验收项 | 验证方式 | 通过标准 | 当前状态 |
|---|---|---|---|
| 创建插件 SearchRun | `POST /api/search-runs/one-time` | 返回 `202` 与 `searchRunId` | 已完成 |
| 目标数量 10-500 | API 校验 | 超出范围返回校验错误 | 已完成 |
| 插件提交候选人 | `POST /api/plugin/search-runs/:id/candidates` | 返回已接收或处理结果 | 基础已完成 |
| 异步聚合 | 集成测试 | 30 秒窗口或 20 条触发评估 | 待实现 |
| SearchRun 取消 | API + 前端 | Running 可取消，插件停止提交 | 待实现 |

## 4. 候选人、筛选与评估

| 验收项 | 验证方式 | 通过标准 | 当前状态 |
|---|---|---|---|
| 单次 SearchRun 内去重 | 自动化测试 | 重复 fingerprint 只保留一条 | 已完成 |
| 跨 SearchRun 去重 | 集成测试 | 同 JobProfile 汇总去重 | 待实现 |
| 硬筛优先 | 自动化测试 | 硬筛淘汰者不进入 AI 评估 | 已完成 |
| 配置驱动硬筛 | 集成测试 | 淘汰/合格规则由配置驱动 | 待实现 |
| AI 推荐结论 | 契约测试 | 输出推荐/待定/不推荐 | 待实现 |
| 重新评估 | 集成测试 | 覆盖旧业务评估结果 | 待实现 |

## 5. 附件

| 验收项 | 验证方式 | 通过标准 | 当前状态 |
|---|---|---|---|
| 插件上传附件 | API 测试 | 20MB 内附件可上传 | 待实现 |
| 重复上传覆盖 | API 测试 | 同候选人只保留最新附件 | 待实现 |
| 附件下载 | API + 前端 | Web 用户可下载 | 待实现 |

## 6. 前端

| 验收项 | 验证方式 | 通过标准 | 当前状态 |
|---|---|---|---|
| 运维工作台 | 浏览器检查 | 登录、创建、提交、查询可见 | 已完成 |
| JobProfile 页面 | 产品验收 | 支持列表、编辑、确认、详情 | 待实现 |
| 版本与重评估页面 | 产品验收 | 支持历史版本和重评估入口 | 待实现 |
| SearchRun 详情 | 产品验收 | 展示状态、进度、候选人、审计 | 待实现 |
| 匹配详情弹窗 | 产品验收 | 展示推荐结论和三类要点 | 待实现 |

## 7. 文档与部署

| 验收项 | 验证方式 | 通过标准 | 当前状态 |
|---|---|---|---|
| OpenAPI | 文件检查 | 覆盖现有 API 和插件 API | 待实现 |
| 插件协议 | 文件检查 | 协议字段和错误码完整 | 已完成 |
| 部署手册 | 文件检查 | 单机 Caddy + Compose 可执行 | 已完成 |
| 运维手册 | 文件检查 | 覆盖备份、恢复、排障 | 已完成 |
