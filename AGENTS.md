...
任何功能、修复、重构、接口、数据模型、前端交互、AI 能力、插件协议、部署或运维相关修改，必须先执行文档驱动检查，再进入代码实现。

### 0.1 修改前必读顺序

每次修改前，必须按以下顺序阅读相关文档：

1. 需求基线文档：确认本次修改是否符合产品范围、业务规则、用户可见行为和阶段目标。
2. 技术设计文档：确认本次修改是否符合分层架构、状态机、接口契约、数据流、AI 审计、权限和安全边界。
3. TODO 文档：确认本次修改是否对应已有待办、是否需要新增待办、拆分待办或更新待办状态。
4. 验收文档：确认本次修改对应的验收项、验证方式和通过标准；若验收标准缺失，必须先补充验收项。
5. 现有代码：在文档约束下阅读已有实现，优先复用现有领域对象、类型、接口和测试策略。

不得在未阅读相关需求文档、技术设计文档、TODO 文档和验收文档的情况下直接改代码。
...
- 新增、删除或改变业务能力。
- 改变 JobProfile、SearchRun、Candidate、HardFilter、AIAssessment、OriginalSourceLink 等核心领域规则。
- 改变状态机、权限、审计、幂等、错误处理或安全边界。
- 改变 API 请求、响应、错误码、认证方式或插件协议。
- 改变 AI 输入、输出、prompt version、agent version、审计字段或校验规则。
- 改变硬筛维度、匹配规则、配置结构或 trace 展示。
- 改变前端页面、字段、交互、文案或用户可见状态。
- 新增、完成、取消、拆分或调整 TODO。
- 新增、完成、取消、拆分或调整验收项。
- 改变验收方式、通过标准、测试命令或验收状态。
- 改变部署、运维、环境变量、备份或发布流程。

以下情况通常不需要修改需求或技术文档，但仍需说明判断理由：

- 不改变行为的类型修复。
- 不改变架构的局部代码整理。
- 测试修复或测试补充。
- 仅修复明显实现错误，且现有文档已经覆盖正确行为和验收标准。

### 0.4 文档与代码落地顺序

涉及文档修改时，必须按以下顺序执行：

1. 更新需求文档，明确用户可见行为、业务规则和阶段范围。
2. 更新技术设计文档，明确架构、接口、数据流、边界和失败处理。
3. 更新 TODO 文档，明确新增、完成、后置或取消的待办。
4. 更新验收文档，明确验收项、验证方式、通过标准和当前状态。
5. 修改代码。
6. 运行验收文档中对应的验证命令或测试场景。
7. 汇报时说明读取了哪些文档、修改了哪些文档、验收项如何更新、运行了哪些验证。

不得先改代码再补文档，除非用户明确要求只做临时实验；临时实验不得进入主分支。

### 0.5 验收联动规则

任何可交付修改都必须能映射到验收文档中的至少一个验收项。

- 如果已有验收项覆盖本次修改，必须在实现后更新其当前状态或补充验证记录。
- 如果没有验收项覆盖本次修改，必须先新增验收项，再实现代码。
- 如果需求或技术设计改变导致原验收项失效，必须同步修改验收项。
- 如果某项能力决定后置，必须在验收文档中标记为后置，并在 TODO 文档中保留追踪项。
- 如果某项验收从“后置”进入实现，必须先更新 TODO 和验收文档，再开始代码修改。
...
开始代码修改前，必须先完成 `## 0. 文档驱动实施流程` 中的需求、技术设计、TODO 和验收检查。
